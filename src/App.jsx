...
/* =========================================================
   ...
   One connected app: Supabase auth, Supabase-backed data, live pages.
========================================================= */
...
export default function VoltApp() {
  const [phase, setPhase] = useState("splash"); // splash | auth | setup | app
  ...
  const loadForUser = async (uid) => {
    try {
      setLoadError("");
      const settings = await Data.fetchSettings(uid);
      if (!settings) {
        setDb({ settings: null, products: [], transactions: [], debts: [] });
        setPhase("setup");
        return;
      }
      const [products, transactions, debts] = await Promise.all([
        Data.fetchProducts(uid), Data.fetchTransactions(uid), Data.fetchDebts(uid),
      ]);
      setDb({ settings, products, transactions, debts });
      setPhase("app");
    } catch (e) {
      console.error(e);
      setLoadError("Couldn't load your data. Check your connection and try again.");
    }
  };

  // boot: splash → check auth session → auth, setup, or app
  useEffect(() => {
    let alive = true;
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const start = Date.now();
    if (!reduce) setTimeout(() => alive && setSplashOut(true), 1800);
    const minSplash = reduce ? 0 : 2400;

    const bootSettle = (nextSession) => {
      const wait = Math.max(0, minSplash - (Date.now() - start));
      setTimeout(() => {
        if (!alive) return;
        bootedRef.current = true;
        setSession(nextSession);
        if (!nextSession) setPhase("auth");
        else loadForUser(nextSession.user.id);
      }, wait);
    };

    supabase.auth.getSession().then(({ data }) => bootSettle(data.session));

    const { data: sub } = supabase.auth.onAuthStateChange((_event, newSession) => {
      if (!alive || !bootedRef.current) return; // ignore events fired before initial boot resolves
      setSession(newSession);
      if (!newSession) { setDb(null); setPhase("auth"); }
      else { loadForUser(newSession.user.id); }
    });
    return () => { alive = false; sub.subscription.unsubscribe(); };
  }, []);

  /* ---------- shared mutations (all write to Supabase, then update local state) ---------- */
  const addTransaction = async (t) => {
    const inserted = await Data.insertTransaction(userId, t);
    setDb((d) => ({ ...d, transactions: [inserted, ...d.transactions] }));
  };

  const recordSale = async (product) => {
    const updated = await Data.updateProduct(product.id, { qty: product.qty - 1 });
    const inserted = await Data.insertTransaction(userId, {
      type: "income", reason: `Sold ${product.name}`, amount: product.price,
      profit: product.price - product.cost, auto: true, productId: product.id,
    });
    setDb((d) => ({
      ...d,
      products: d.products.map((p) => (p.id === product.id ? updated : p)),
      transactions: [inserted, ...d.transactions],
    }));
  };

  const addStock = async (product, n) => {
    const updated = await Data.updateProduct(product.id, { qty: product.qty + n });
    setDb((d) => ({ ...d, products: d.products.map((p) => (p.id === product.id ? updated : p)) }));
  };

  const saveNewProduct = async (prod) => {
    const inserted = await Data.insertProduct(userId, prod);
    setDb((d) => ({ ...d, products: [inserted, ...d.products] }));
    return inserted;
  };

  const saveProduct = async (id, patch) => {
    const updated = await Data.updateProduct(id, patch);
    setDb((d) => ({ ...d, products: d.products.map((p) => (p.id === id ? updated : p)) }));
  };
  ...
  const adjustStock = async (product, n, reason) => {
    const updated = await Data.updateProduct(product.id, { qty: Math.max(0, product.qty - n) });
    await Data.insertStockAdjustment(userId, {
      productId: product.id, qtyRemoved: n, reason, valueWrittenOff: product.cost * n,
    });
    setDb((d) => ({ ...d, products: d.products.map((p) => (p.id === product.id ? updated : p)) }));
  };
  ...
  const addDebt = async (debt) => {
    const inserted = await Data.insertDebt(userId, debt);
    setDb((d) => ({ ...d, debts: [...d.debts, inserted] }));
  };

  const settleDebt = async (debt) => {
    await Data.deleteDebt(debt.id);
    const inserted = await Data.insertTransaction(userId,
      debt.direction === "owe"
        ? { type: "expense", reason: `Paid ${debt.name}`, amount: debt.amount }
        : { type: "income", reason: `${debt.name} paid you`, amount: debt.amount }
    );
    setDb((d) => ({
      ...d,
      debts: d.debts.filter((x) => x.id !== debt.id),
      transactions: [inserted, ...d.transactions],
    }));
  };

  const saveSettings = async (patch) => {
    const updated = await Data.updateSettings(userId, patch);
    setDb((d) => ({ ...d, settings: updated }));
  };
  ...
  if (phase === "auth") return <Shell><Auth /></Shell>;
  if (phase === "setup") {
    return (
      <Shell>
        <Setup
          userId={userId}
          onDone={(settings) => { setDb({ settings, products: [], transactions: [], debts: [] }); setPhase("app"); }}
        />
      </Shell>
    );
  }
      ...
      <Shell>
        <div className="center">
          <div className="card authcard rise-now">
            <div className="shead red">Something went wrong</div>
            <div className="err">{loadError}</div>
            <button className="btn big" onClick={() => loadForUser(userId)}>Try again</button>
          </div>
        </div>
      </Shell>
    );
  }
  if (!db) return <Shell><Splash out={true} name="VOLT ELECTRONICS" /></Shell>;
    ...
    <Shell>
      <header className="hdr">
        ...
        <div className="hbtns">
          ...
          <button className="iconbtn" onClick={() => supabase.auth.signOut()} aria-label="Sign out">🔒</button>
        </div>
      </header>
      ...
      {showSettings && <Settings db={db} session={session} saveSettings={saveSettings} close={() => setShowSettings(false)} />}
    </Shell>
  );
}
...
/* =========================================================
   AUTH (sign up / log in) & SETUP (first run)
========================================================= */
function Auth() {
  ...
  return (
    <div className="center">
      <div className="card authcard rise-now">
        <div className="shead" style={{ textAlign: "center" }}>{mode === "login" ? "Log in" : "Create your account"}</div>
        <div className="fcol">
        ...
        {msg && <div className="note b">{msg}</div>}
...
function Setup({ userId, onDone }) {
  const [name, setName] = useState("VOLT ELECTRONICS");
  ...
  const go = async () => {
    ...
    try {
      const settings = await Data.createSettings(userId, {
        storeName: name.trim().toUpperCase(), currency, openingCash: Number(cash) || 0,
      });
      onDone(settings);
    } catch (e) {
      console.error(e);
      setErr("Couldn't save your store settings. Try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="center">
      <div className="card authcard rise-now">
        ...
        {err && <div className="err">{err}</div>}
        <button className="btn big" onClick={go} disabled={saving}>{saving ? "Setting up…" : "Open my store"}</button>
      </div>
    </div>
  );
}
...
/* =========================================================
   HOME
========================================================= */
function Home({ db, cur, addDebt, settleDebt }) {
  ...
  const saveDebt = async () => {
    ...
    setBusy(true); setErr("");
    try {
      await addDebt({ direction: addingDebt, name: dName.trim(), amount: Number(dAmt), due: dDue });
  ...
  const handleSettle = async (d) => {
    try { await settleDebt(d); } catch (e) { console.error(e); }
  };
    ...
    <div className="stack">
        ...
        {owe.map((d) => (
          <div className="row" key={d.id}>
            ...
            <div className="rright">
              ...
              <button className="tinybtn" onClick={() => handleSettle(d)}>Paid ✓</button>
            </div>
          </div>
        ))}
      </div>
        ...
        {owed.map((d) => (
          <div className="row" key={d.id}>
            ...
            <div className="rright">
              ...
              <button className="tinybtn" onClick={() => handleSettle(d)}>Received ✓</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {addingDebt && (
        <Overlay close={() => setAddingDebt(null)}>
          ...
          {err && <div className="err">{err}</div>}
          <div className="mrow">
            <button className="btn" onClick={saveDebt} disabled={busy}>{busy ? "Saving…" : "Save"}</button>
...
function Scan({ db, cur, recordSale, addStock, saveNewProduct }) {
  ...
  const doSale = async () => {
    ...
    try {
      await recordSale(found);
      setFlash(`✓ Sold ${found.name} — ${fmt(found.price, cur)} (+${fmt(found.price - found.cost, cur)} profit)`);
      reset(false);
    } catch (e) {
      console.error(e);
      setScanError("Couldn't record the sale. Try again.");
    }
  };

  const doAddStock = async (n) => {
    if (!found) return;
    try {
      await addStock(found, n);
      setFlash(`✓ Added ${n} × ${found.name} to stock`);
      reset(false);
    } catch (e) {
      console.error(e);
      setScanError("Couldn't update stock. Try again.");
    }
  };

  const doSaveNewProduct = async () => {
    if (!form.barcode || !form.name || form.cost === "" || form.price === "") return;
    try {
      const prod = {
        barcode: form.barcode, name: form.name, brand: form.brand, category: form.category,
        cost: Number(form.cost), price: Number(form.price), qty: Number(form.qty) || 0,
        alertAt: Number(form.alertAt) || 0, supplier: form.supplier,
      };
      const saved = await saveNewProduct(prod);
      setFlash(`✓ ${saved.name} added to inventory (${saved.qty} in)`);
      reset(false);
    } catch (e) {
      console.error(e);
      setScanError("Couldn't save the product — that barcode might already be in your inventory.");
    }
  };
    ...
    <div className="stack">
      ...
      {found && (
        <section className="card hit">
          ...
          {mode === "sell" ? (
            ...
            <div className="mrow">{[1, 5, 10].map((n) => <button key={n} className="btn" onClick={() => doAddStock(n)}>+{n}</button>)}</div>
          )}
      ...
      {notFound && (
        <section className="card">
          ...
          <div className="mrow">
            <button className="btn" onClick={doSaveNewProduct}>Save product</button>
...
/* =========================================================
   MONEY
========================================================= */
function Money({ db, cur, addTransaction, removeTransaction }) {
  ...
  const add = async () => {
    ...
    try {
      await addTransaction({ type, reason: reason.trim(), amount: amt });
      setReason(""); setAmount("");
  ...
  const remove = async (id) => {
    try { await removeTransaction(id); } catch (e) { console.error(e); }
  };
  ...
  return (
    <div className="stack">
      ...
      <section className="card">
        ...
        <button className={`btn big ${type === "expense" ? "red-bg" : ""}`} onClick={add} disabled={busy}>
...
function Stock({ db, cur, saveProduct, deleteProduct, adjustStock, addStock }) {
  ...
  const saveEdit = async () => {
    ...
    setEditErr("");
    try {
      await saveProduct(editing.id, {
        name: editing.name, barcode: editing.barcode, brand: editing.brand, category: editing.category,
        cost: Number(editing.cost), price: Number(editing.price),
        qty: Number(editing.qty) || 0, alertAt: Number(editing.alertAt) || 0, supplier: editing.supplier,
      });
      setEditing(null);
    } catch (e) {
      console.error(e);
      setEditErr("Couldn't save — that barcode might already be used by another product.");
    }
  };
  ...
  return (
    <div className="stack">
      ...
      {confirmDelete && (
        <Overlay close={() => { setConfirmDelete(null); setDeleteErr(""); }}>
          ...
          <div className="mrow">
            <button className="btn red-bg" onClick={async () => {
              try {
                await deleteProduct(confirmDelete.id);
                setConfirmDelete(null); setDeleteErr("");
              } catch (e) {
                console.error(e);
                setDeleteErr("Can't delete — this product has recorded sales history. Try setting its stock to 0 instead.");
              }
            }}>Yes, delete it</button>
            <button className="btn ghost" onClick={() => { setConfirmDelete(null); setDeleteErr(""); }}>Keep it</button>
          </div>
        </Overlay>
      )}
        ...
        <Overlay close={() => setAdjusting(null)}>
          ...
          <div className="mrow">
            <button className="btn red-bg" onClick={async () => {
              ...
              try {
                await adjustStock(adjusting, n, adjReason);
                setAdjusting(null);
...
/* =========================================================
   SETTINGS
========================================================= */
function Settings({ db, session, saveSettings, close }) {
  ...
  const save = async () => {
    setErr(""); setSaving(true);
    try {
      await saveSettings({ storeName: s.storeName.toUpperCase(), currency: s.currency, openingCash: Number(s.openingCash) || 0 });
      close();
    } catch (e) {
      console.error(e);
      setErr("Couldn't save settings. Try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Overlay close={close}>
      ...
      {err && <div className="err">{err}</div>}
      <div className="note">Signed in as <b>{session.user.email}</b></div>
      <div className="mrow">
